...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    public Calculate initCalculate() {
        return Calculate.CalculateBuilder
                ...
                .withBtn_sqrt(findViewById(R.id.btn_sqrt))
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            ...
            case R.id.btn_sqrt:
                getResult();
                if(tx_reslut.getText().charAt(0)=='-')
                { Toast.makeText(MainActivity.this,
                            "错误，负数不能开平方根",
                            Toast.LENGTH_SHORT).show();
                    tx_reslut.setText("0");
                    break;}
                if(tx_reslut.getText().charAt(0)=='0'&& tx_reslut.getText().length()==1){
                    tx_reslut.setText("0");
                    break;
                }
                tx_reslut.setText(sqrt(new BigDecimal(tx_reslut.getText().toString())).toString());
                break;
            case R.id.btn_AC:
                tx_reslut.setText("0");
                break;
            case R.id.btn_percent:
                if (matcher.find()&&tx_reslut.getText().charAt(0)!='-') {
                    tx_reslut.setText("-"+s.substring(0, s.length() - matcher.group().length()) + ((matcher.group())));
                }
                else{
                    tx_reslut.setText(s.substring(1, s.length() - matcher.group().length()) + ((matcher.group())));
                }
                break;
            case R.id.btn_equal:
                getResult();
    ...
    public void getResult() {
        ...
        TextView tx_reslut = findViewById(R.id.tx_result);
        if(tx_reslut.getText().charAt(0)=='-'){
            tx_reslut.setText("0"+tx_reslut.getText());
        }
